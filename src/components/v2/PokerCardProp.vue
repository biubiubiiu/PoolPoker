...
<template>
  <button class="poker-prop-card" :class="{ 'is-dimmed': isDimmed, 'is-elevated': isElevated, 'is-discarding': isDiscarding, 'red-ink': red }"
    :disabled="disabled" :aria-label="`${card.rank}${card.suit}，${card.ballNumber}号球，${isDimmed ? '免打牌' : '点击出牌'}`"
    @click="emit('click', card)">
    <span class="card-face">
      <span class="card-corner"><strong>{{ joker ? '王' : card.rank }}</strong><span>{{ joker ? '✦' : card.suit }}</span>
...
<style scoped>
.poker-prop-card { width: 82px; height: 124px; flex-shrink: 0; color: #20352d; cursor: pointer; position: relative; border-radius: 7px; transform-style: preserve-3d; transition: transform 480ms cubic-bezier(.2,.8,.2,1), opacity 400ms; text-align: left; }
...
@media (prefers-reduced-motion:reduce) { .poker-prop-card { transition-duration: 1ms; } .is-discarding { transform: none; } }
</style>

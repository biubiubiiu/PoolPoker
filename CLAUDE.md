...
- **Two room shapes.** `ServerRoom` holds server-internal fields (`deck`, `accidentalBalls`, `lastWinnerUserId`, `lastTurnOrder`); `Room` is the sanitized client view. `getClientRoomState` (in `roomManager.ts`) clips `cards`/`pocketedCards` to only the requesting user (or everyone once the room is `finished`) — this is the mechanism that prevents leaking other players' hands. Never add a sensitive field to the client view without clipping it here.

...
- **iOS & Mobile UI Adaptations.** UI components use `.safe-area-spacing` and CSS `env(safe-area-inset-top)` / `env(safe-area-inset-bottom)` / `env(safe-area-inset-left)` / `env(safe-area-inset-right)` (`--sat`, `--sab`, `--sal`, `--sar`) to account for iPhone notches, dynamic islands, and home indicators. Native dialog helpers (`showConfirm`, `showAlert` in `src/utils/dialog.ts`) delegate to `@tauri-apps/plugin-dialog` under Tauri runtime (iOS/Android) and fallback to `window.confirm`/`window.alert` in Web. `RoomLobby.vue` exposes server URL configuration when `isTauriEnv` is true. Card and ball assets enforce `color-scheme: only light !important` and `forced-color-adjust: none !important` in `src/styles/main.css` to prevent auto dark mode color inversion on iOS Safari / WebKit.
- **Billiards table asset contract.** `assets/models/billiards_table.blend` is the editable source and is tracked by Git LFS; `public/models/billiards_table.glb` is the web runtime asset. Keep the six `PocketTarget_*` objects when exporting: `ThreeBilliardsArena` reads them to target pocket animations. Export only the finished table collection, not preview cameras, lights, sample balls, floor, or hidden historical objects. Preview renders in `assets/models/previews/` are local-only. See `docs/table_asset_pipeline.md` before changing the model.
- **Git LFS.** `.gitattributes` tracks `*.blend` through LFS and `.husky/pre-push` uploads LFS objects on push. Before adding or changing Blender source assets, run `git lfs install --local --skip-repo` once on a new clone and use `git lfs ls-files` to verify that the `.blend` is an LFS object.
...
- `docs/table_asset_pipeline.md` covers the Blender source, GLB export settings, pocket anchors, and asset verification flow.
